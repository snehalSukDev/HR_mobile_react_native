...
const HolidaysScreen = ({ currentUserEmail, currentEmployeeId }) => {
  ...
  const { upcomingHolidays, pastHolidays } = useMemo(() => {
    ...
    [...filtered].sort((a, b) => new Date(a.holiday_date) - new Date(b.holiday_date))
      .forEach(h => {
...
const renderList = (data, title) => (
  <View style={styles.holidaysSubSection}>
    ...
    {data.length > 0 ? (
      <View style={styles.table}>
        ...
        {data.map((h) => (
          <View key={h.id} style={styles.tableRow}>
            <Text style={[styles.tableCol, styles.colDate]}>{formatDate(h.holiday_date)}</Text>
            <Text style={[styles.tableCol, styles.colDescription]}>{h.description || '—'}</Text>
            <Text style={[
              styles.tableCol,
              styles.colType,
      ...
      <Text style={styles.noDataText}>No {title.toLowerCase()} for {currentYear}.</Text>
    )}
  </View>
);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      {currentYear <= new Date().getFullYear() && renderList(pastHolidays, "Past Holidays")}
    </ScrollView>
  );
};
...
const styles = StyleSheet.create({
  ...
  navButton: {
    padding: 8, borderRadius: 5, marginLeft: 5, backgroundColor: '#e3f2fd'
  },
...
filterLabel: {
  ...
  subSectionTitle: {
    ...
    borderBottomWidth: 1, borderBottomColor: '#eee', paddingBottom: 5, marginBottom: 10
  },
  noDataText: {
    color: '#777', textAlign: 'center', padding: 10,
    backgroundColor: '#fff', borderRadius: 8, elevation: 2
  },
  holidayEntry: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    backgroundColor: '#fff', padding: 12, borderRadius: 8, marginBottom: 10, elevation: 1
  },
  holidayEntrySmall: {
  ...
  typePublicHolidayText: {
    color: '#1976d2', fontSize: 12, fontWeight: 'bold', textTransform: 'uppercase'
  },
  typeWeeklyOff: { backgroundColor: '#e8f5e9' },
  typeWeeklyOffText: {
    color: '#388e3c', fontSize: 12, fontWeight: 'bold', textTransform: 'uppercase'
  },
  table: {
  borderWidth: 1,
  borderColor: '#ddd',
